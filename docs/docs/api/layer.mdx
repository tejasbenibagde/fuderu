...
| `alphaLock?` | `boolean`   | Clip edits to non-transparent pixels (defaults to `false`). |
...
| `alphaLock` | `boolean`                             | When `true`, drawing/filling is clipped to opaque pixels.              |
| `locked`    | `boolean`                             | When `true`, prevents edits, clearing, or deletion of the layer.       |
