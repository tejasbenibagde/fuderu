...
export const toHashColor = (color: string): string => {
  ...
  if (HEX_LONG_REGEX.test(normalized)) {
    COLOR_CACHE.set(normalized, normalized);
  ...
  const shortHexMatch = normalized.match(HEX_SHORT_REGEX);
  ...
  if (shortHexMatch) {
    ...
    COLOR_CACHE.set(normalized, expanded);
  ...
  if (rgbMatch) {
    ...
    COLOR_CACHE.set(normalized, hex);
