...
charcoalBrush.useModule(new DynamicShapeModule({
  sizeJitter: 0.5,
  angleJitter: 0.3,
  roundJitter: 0.4,
  minRoundness: 0.2,
...
charcoalBrush.useModule(new DynamicTransparencyModule({
  opacityJitter: 0.4,
  flowJitter: 0.3,
}));

charcoalBrush.useModule(new SpreadModule({
  spreadRange: 0.2,
  count: 2,
}));

charcoalBrush.useModule(new PatternModule({
  scale: 0.5,
  contrast: 15,
...
<input type="range" id="spread-amount" min="0" max="1" step="0.01" value="0.4">
```
