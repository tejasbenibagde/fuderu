...
- `config.spacing` controls spacing-based interpolation. Smaller values place
  stamps closer together.
- `blendMode` maps to `CanvasRenderingContext2D.globalCompositeOperation`.
- `filter` maps to `CanvasRenderingContext2D.filter`.

`isSpacing` is still exposed for compatibility, but current spacing behavior is
driven by `config.spacing`.
