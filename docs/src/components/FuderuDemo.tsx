...
export default function FuderuDemo({
    ...
    const [blendMode, setBlendMode] = useState<CanvasRenderingContext2D["globalCompositeOperation"]>(
        "source-over"
    );
    const [filter, setFilter] = useState<CanvasRenderingContext2D["filter"]>("none");
    ...
    useEffect(() => {
        ...
        canvas.loadConfig({
            ...
            opacity: 0.95,
            flow: 0.9,
            rotation: mode === "image" ? { mode: rotationMode, smoothing: 0.15 } : undefined,
        });
    ...
    }, [color, size, spacingValue, isSmooth, isSpacingEnabled, blendMode, filter, rotationMode, isEraser, mode]);
    ...
    const toggleModule = async (
        enabled: boolean,
        factory: () => DynamicShapeModule | DynamicTransparencyModule | SpreadModule | PatternModule,
        ref: React.MutableRefObject<DynamicShapeModule | DynamicTransparencyModule | SpreadModule | PatternModule | null>,
            ...
            {!compact && (
                <div className={styles.controls}>
                    <div className={styles.controlRow}>
                        <button className={styles.button} onClick={handleClear} type="button">
                            Clear
                        </button>
                        <button className={styles.button} onClick={handleUndo} type="button">
                            Undo
                        </button>
                        <button className={styles.button} onClick={handleRedo} type="button">
                            Redo
                        </button>
                        <button className={styles.button} onClick={handleEraserToggle} type="button">
                    ...
                    <div className={styles.controlRow}>
                        ...
                        <label className={styles.controlLabel}>
                            ...
                            <input
                                ...
                                step="0.05"
                                value={spacingValue}
                                onChange={(event) => setSpacingValue(Number(event.target.value))}
                            />
                        </label>
                    </div>
                    {isImageMode && (
                        <div className={styles.controlRow}>
                            ...
                            <button className={styles.button} onClick={handleRemoveImage} type="button">
                                Remove image
                            </button>
                        ...
                        <div className={styles.controlRow}>
                            <label className={styles.controlLabel}>
                                ...
                                <select
                                    ...
                                    value={rotationMode}
                                    onChange={(event) =>
                                        setRotationMode(event.target.value as "fixed" | "flow" | "random")
                                    }
                                >
                        ...
                        <div className={styles.controlRow}>
                            ...
                            <label className={styles.checkboxLabel}>
                                <input
                                    ...
                                    onChange={(event) => setIsSpacingEnabled(event.target.checked)}
                                />
                                Spacing
                            </label>
                            <label className={styles.checkboxLabel}>
                                <input
                                    ...
                                    onChange={(event) => setIsPressureEnabled(event.target.checked)}
                                />
                            ...
                            <label className={styles.controlLabel}>
                                ...
                                <select
                                    ...
                                    onChange={(event) =>
                                        setBlendMode(
                                            event.target.value as CanvasRenderingContext2D["globalCompositeOperation"],
                                        )
                                    }
                                >
                            ...
                            <label className={styles.controlLabel}>
                                ...
                                <select
                                    className={styles.selectInput}
                                    value={filter}
                                    onChange={(event) => setFilter(event.target.value as CanvasRenderingContext2D["filter"])}
                                >
                    ...
                    {isModuleMode && (
                        <div className={styles.controlRow}>
                            ...
                            {isPatternLoading && <span className={styles.status}>Loading pattern…</span>}
                        </div>
                    )}
                </div>
            )}
