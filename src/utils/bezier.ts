...
export const getQuadraticBezierDistance = (
  ...
  for (let i = 1; i <= numSegments; i++) {
    ...
    const point = quadraticBezier(t, p1x, p1y, ptx, pty, p2x, p2y);
...
export const getEquidistantBezierPoints = (
  ...
  for (let i = 1; i <= segments; i++) {
    ...
    const segmentLength = distance(prevX, prevY, point.x, point.y);

    accumulated += segmentLength;

    if (accumulated >= space) {
      const lastPoint =
        points.length > 0 ? points[points.length - 1] : { x: p1x, y: p1y };

      if (
        distanceSquared(lastPoint.x, lastPoint.y, point.x, point.y) >= spacingSq
      ) {
        points.push(point);
        accumulated = 0;
      }
