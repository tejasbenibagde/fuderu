...
describe("getAngle", () => {
  it("should calculate angle correctly", () => {
    expect(getAngle(0, 0, 1, 0)).toBeCloseTo(0);

    expect(getAngle(0, 0, 0, 1)).toBeCloseTo(Math.PI / 2);
  });
});

describe("getPointBetween", () => {
  it("should return midpoint by default", () => {
    expect(getPointBetween(0, 0, 10, 10)).toEqual({
      x: 5,
      y: 5,
    });
  });
  ...
  it("should interpolate correctly", () => {
    expect(getPointBetween(0, 0, 10, 10, 0.25)).toEqual({
      x: 2.5,
      y: 2.5,
    });
  });
...
describe("splitDecimal", () => {
  it("should split decimals correctly", () => {
    expect(splitDecimal(3.2)).toEqual([1, 1, 1, 0.2]);
  });

  it("should split using custom unit", () => {
    expect(splitDecimal(4.1, 2)).toEqual([2, 2, 0.1]);
  });

  it("should return empty array for invalid unit", () => {
    expect(splitDecimal(5, 0)).toEqual([]);
  });
});
