...
class GoodModule implements Module {
  ...
  onChangePoint(point: Point, config: BrushBasicConfig): Point {
    this.tempPoint.x = point.x + (Math.random() - 0.5) * 2;
    this.tempPoint.y = point.y + (Math.random() - 0.5) * 2;
    this.tempPoint.pressure = point.pressure;
    this.tempPoint.config = structuredClone(config);
...
- Explore the [Module API Reference](/docs/guides/modules) for complete interface details
- Check out the [Performance Guide](/docs/guides/performance) for broader optimization strategies
- Try the [Editor Demo](/editor) to experiment with layers and brush behavior
- Review the [Quick Start](/docs/getting-started/quick-start) for a simple end-to-end example
