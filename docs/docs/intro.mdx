...
Fuderu 1.3.2 expands beyond brush rendering into a complete, production-ready canvas architecture: featuring first-class document persistence with lock state serialization, layer-level Alpha Lock and Layer Lock, native scanline flood fill, shape primitives, raster text, pointer ID gesture safety, and typed state event subscriptions.
